...
public abstract class BoxModel {
	...
	private boolean sealed, bottom, corner; // to check if the box is sealed,
											// has reinforced corners or bottoms
	protected double cost; // cost of the box
	private double area; // area of the box 2*(height * width) + 2*(height *
	...
	public BoxModel(double width, double height, double length, int grade, int boxColour, boolean sealed,
			boolean bottom, boolean corner) {
		this.width = width;
		this.height = height;
		this.length = length;
		this.grade = grade;
	...
	public void calculateCost() {
		cost = calculateArea() * gradeCost(); // sets the current cost to be
												// equal to area * price
		cost += cost / 100 * extraCost(); // adds any extra cost to the cost
											// variable
